<template>
	<div class="fullPath">
		<div v-for="(item, index) in followList" :key="index" class="card  fill-white tiny-round  border">
			<!-- 展示顶部信息（作者+时间） -->
			<!-- 判断如果有作者信息，展示作者+回答了该问题，如果是没有的，展示出多少人阅读该文章 -->
			<div class="ml-3 mt-2">
				<div v-if=" item.target.author.name">
					<p class="top-text-set">{{ item.target.author.name}}回答了该问题·{{item.target.author.gender}}小时之前</p>
				</div>
				<div v-else>
					<p class="top-text-set ">{{item.target.created_time}}人阅读了该文章</p>
				</div>
			</div>
			<div class="content-wrapper p-2">
				<h3 v-if="item.target.question == null">{{ item.target.title }}</h3>
				<h3 v-else class="ml-2">{{ item.target.question.title }}</h3>
				<!-- 初始状态都是收起来的 -->
	...
	created() {
		this.axios.get('/api/follow').then(res => {
			...
			for (var i = 0; i < this.followList.length; i++) {
				...
				//将富文本中的图片处理大小
				this.followList[i].target.content = this.followList[i].target.content.replace(regex, `<img style="max-width: 200px; height: 150px"`);
			}
		});
	}
};
